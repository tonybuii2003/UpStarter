...
export default function SimplifiedMatchStartup({ 
  compact = false, 
  name = "Startup Name", 
  logo_path = "", 
  ...
  return (
    <LinearGradient style={styles.card} colors={['#6A8DFF', '#1E4D91']}>
      ...
      {/* --- About Section (show truncated in compact mode) --- */}
      <View style={styles.aboutBox}>
        <Text style={styles.aboutTitle}>About us:</Text>
        <Text style={styles.aboutText}>
          {compact ? 
            (about_content.length > 320 ? about_content.substring(0, 320) + '...' : about_content) 
            : about_content
          }
        </Text>
      </View>

      {/* --- Business Plan Section (only show when not compact) --- */}
      {!compact && (
